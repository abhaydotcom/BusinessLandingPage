    ...
    <section id="about" className="w-full bg-gray-50 py-16 px-4 md:px-8">
      <div className="max-w-6xl mx-auto">
        ...
        className="grid md:grid-cols-3 grid-cols-1 gap-8">
          ...
          <motion.div  variants={fadeIn('left', 0.3)} className="col-span-2 grid grid-cols-1 md:grid-cols-2 justify-between gap-8">
            {features.map((feature, idx) => (
              <motion.div 
               variants={fadeIn('up', 0.3 * (idx + 1))}
              key={idx} className="flex items-start space-x-4">
                <motion.div 
                variants={fadeIn('right', 0.4 * (idx + 1))}
                className="w-12 h-12 flex items-center justify-center rounded-lg">
                  {feature.icon}
                </motion.div>

                <motion.div 
                variants={fadeIn('left', 0.4 * (idx + 1))}>
                  <motion.h3 
